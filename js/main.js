...
function renderSummary() {
  ...
  if (classData.goalGrade !== '' && !Number.isNaN(Number(classData.goalGrade))) {
    html += `<p><strong>Saved goal grade:</strong> ${round2(Number(classData.goalGrade))}%</p>`;
  }
  if (total !== 100) {
  ...
  html += `<table>
    <tr><th>Category</th><th>Weight</th><th>Points</th><th>Category Average</th><th>Weighted Contribution (Current)</th><th>Correction Cap</th></tr>`;
...
function renderInsightsPanel() {
  el('insightsOutput').innerHTML = renderInsights(classData);
}

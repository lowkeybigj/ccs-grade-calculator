...
function saveGradeEdits(categoryIndex, gradeIndex) {
  ...
  const correctedEarned = correctionEnabled
    ? (correctedRaw.trim() === '' ? earned : Number(correctedRaw))
    : null;

  if (correctionEnabled && Number.isNaN(correctedEarned)) return alert('Enter a valid corrected score.');
